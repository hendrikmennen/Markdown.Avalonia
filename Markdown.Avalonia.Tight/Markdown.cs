...
#pragma warning restore CS0618
        ...
        private void SetupParser()
        {
            ...
            topBlocks.Add(BlockParser.New(_codeBlockBegin, nameof(CodeBlocksWithLangEvaluator),
                CodeBlocksWithLangEvaluator));

            if (info.EnableContainerBlockExt)
                topBlocks.Add(BlockParser.New(_containerBlockFirst, nameof(ContainerBlockEvaluator),
            ...
            if (info.EnableTableBlock) subBlocks.Add(BlockParser.New(_table, nameof(TableEvalutor), TableEvalutor));

            if (info.EnableNoteBlock) subBlocks.Add(BlockParser.New(_note, nameof(NoteEvaluator), NoteEvaluator));

            subBlocks.Add(BlockParser.New(_indentCodeBlock, nameof(CodeBlocksWithoutLangEvaluator),
                CodeBlocksWithoutLangEvaluator));


            // inline parser

            inlines.Add(InlineParser.New(_codeSpan, nameof(CodeSpanEvaluator), CodeSpanEvaluator));
            inlines.Add(InlineParser.New(_imageOrHrefInline, nameof(ImageOrHrefInlineEvaluator),
            ...
            if (StrictBoldItalic)
            {
                ...
                if (info.EnableStrikethrough)
                    inlines.Add(
                        InlineParser.New(_strikethrough, nameof(StrikethroughEvaluator), StrikethroughEvaluator));
            }
                ...
                List<Control> outto)
            {
                for (; parserStart < _blockParsers.Length; ++parserStart)
                {
                    ...
                    for (;;)
                    {
                        ...
                        var rslt = parser.Convert(text, match, status, this, out var parseBegin, out var parserEnd);
        ...
        private Border NoteComment(IEnumerable<CInline> content, TextAlignment? indiAlignment)
        {
            ...
            if (indiAlignment.HasValue) note.TextAlignment = indiAlignment.Value;
        ...
        private static readonly Regex _startNoIndentSublistMarker = new(@"\A" + _extSubseqListMaker,
            RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);

        private static readonly Regex _startQuoteOrHeader = new(@"\A(\#{1,6}[ ]|>|```)",
            RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);

        private static readonly Regex _startNoIndentCommonSublistMarker = new(@"\A" + _commonListMaker,
            RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
        ...
        private static readonly Regex _startNoIndentExtSublistMarker = new(@"\A" + _extSubseqListMaker,
            RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
        ...
        {
            return ListEvaluator(match, _startNoIndentExtSublistMarker);
        }

        private IEnumerable<Control> CommonListEvaluator(Match match)
        {
            return ListEvaluator(match, _startNoIndentCommonSublistMarker);
        }
        ...
        private IEnumerable<Control> ListEvaluator(Match match, Regex sublistMarker)
        {
            ...
            var countIndent = TextUtil.CountIndent(match.Groups["mkr_i"].Value);

            // whole list
            var whileListLins = match.Groups["whltxt"].Value.Split('\n');
            ...
            foreach (var line in whileListLins)
            {
                ...
                if (isInOuterList) outerListBuildre.Append(line).Append("\n");
            }

            var list = listBulder.ToString();

            var listItems = ProcessListItems(list, markerPattern);
        ...
        private static (TextMarkerStyle, string, int) GetTextMarkerStyle(string markerText)
        {
            ...
            if (Regex.IsMatch(markerText, _markerUL_Box))
                return (TextMarkerStyle.Box, _markerUL_Box, 2);
            if (Regex.IsMatch(markerText, _markerUL_Circle))
                return (TextMarkerStyle.Circle, _markerUL_Circle, 2);
            if (Regex.IsMatch(markerText, _markerUL_Square))
                return (TextMarkerStyle.Square, _markerUL_Square, 2);
            if (Regex.IsMatch(markerText, _markerOL_Number))
                return (TextMarkerStyle.Decimal, _markerOL_Number, 3);
            if (Regex.IsMatch(markerText, _markerOL_LetterLower))
                return (TextMarkerStyle.LowerLatin, _markerOL_LetterLower, 3);
            if (Regex.IsMatch(markerText, _markerOL_LetterUpper))
                return (TextMarkerStyle.UpperLatin, _markerOL_LetterUpper, 3);
            if (Regex.IsMatch(markerText, _markerOL_RomanLower))
                return (TextMarkerStyle.LowerRoman, _markerOL_RomanLower, 3);
            if (Regex.IsMatch(markerText, _markerOL_RomanUpper))
        ...
        private Border TableEvalutor(Match match)
        {
            ...
            foreach (var tableHeaderCell in CreateTableRow(mdtable.Header, 0))
            {
            ...
            // table cell
            foreach (var rowIdx in Enumerable.Range(0, mdtable.Details.Count))
            {
                table.RowDefinitions.Add(new RowDefinition());
                foreach (var cell in CreateTableRow(mdtable.Details[rowIdx], rowIdx + 1))
                {
        ...
        {
            var result =
                ContainerBlockHandler?.ProvideControl(AssetPathRoot, match.Groups[2].Value, match.Groups[3].Value);

            if (result is null)
            {
                var _retVal = CodeBlocksEvaluator(match.Value);
        ...
        private Border? CodeBlocksWithLangEvaluator(string text, Match match, out int parseTextBegin,
